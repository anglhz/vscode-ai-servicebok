...
const operation={lease_token:"lease",customer_key:"customer-key",customer_started_at:new Date().toISOString(),checkout_key:null as string|null,checkout_expires_at:null as number|null,checkout_price_id:null as string|null,checkout_origin:null as string|null,checkout_session_id:null as string|null};
...
beforeEach(()=>{
  ...
  customer="cus_own";Object.assign(operation,{customer_started_at:new Date().toISOString(),checkout_key:null,checkout_session_id:null,checkout_expires_at:null,checkout_price_id:null,checkout_origin:null});
  m.user.mockResolvedValue({id:user});m.lease.mockImplementation((_user,work)=>work({...operation},customer));
  m.operation.mockImplementation((_user,_lease,action,value)=>{if(action==="checkout")Object.assign(operation,{checkout_key:"attempt-key",checkout_price_id:value.price,checkout_origin:value.origin,checkout_expires_at:Math.floor(Date.now()/1000)+3600});return {...operation};});
...
it("Free checkout reuses own customer, fixed monthly price and APP_URL",async()=>{
  ...
  expect(m.customer).not.toHaveBeenCalled();expect(m.checkout).toHaveBeenCalledWith(expect.objectContaining({mode:"subscription",customer:"cus_own",line_items:[{price:"price_monthly",quantity:1}],success_url:"https://servicebok.example/account?checkout=success",cancel_url:"https://servicebok.example/account?checkout=cancelled"}),{idempotencyKey:"servicebok-checkout:attempt-key"});
});
it("creates customer with stable idempotency and saves the relation before checkout",async()=>{customer=null;await startCheckout();expect(m.customer).toHaveBeenCalledWith({metadata:{user_id:user}},{idempotencyKey:"servicebok-customer:customer-key"});expect(m.operation.mock.calls[0]).toEqual([user,"lease","customer",{id:"cus_own"}]);});
